...
class VAEDecoderWrapperSingle(nn.Module):
    ...
    def forward(
        self, z: torch.Tensor, is_first_frame: torch.Tensor, *feat_cache: List[torch.Tensor]
    ):
...
class VAEDecoder3d(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, is_first_frame: torch.Tensor, feat_cache: List[torch.Tensor]
    ):
        idx = 0
        out_feat_cache = []
